...
//
b.INPtoBSF(b.inp,b.bsf);
b.encode(b.bsf, b.com);
b.decode(b.com, b.dec);
b.DECtoOUT(b.dec, b.out);
System.out.println("...");
...
public File dec;
public File out;
public int lc;public int cc;
public CompressionIMG() {inp=null;txt=null;bsf=null;com=null;dec=null;out=null;lc=0;cc=0;} 

public void init(String filename) {
inp=new File("C:\\Users\\bilal.iqbal\\Pictures\\FILES\\sample\\"+filename+".png");
...
com=new File("C:\\Users\\bilal.iqbal\\Pictures\\FILES\\sample\\com.txt");
dec=new File("C:\\Users\\bilal.iqbal\\Pictures\\FILES\\sample\\dec.txt");
out=new File("C:\\Users\\bilal.iqbal\\Pictures\\FILES\\sample\\"+filename+"_.png");
lc=0; cc=0;}

public  void INPtoBSF(File originalFile, File outFile) throws IOException {
INPtoTXT(inp,txt);TXTtoBSF();}

public  void INPtoTXT(File input, File output) throws IOException {
...
int dlc=0;//dist line count
     if((lta+0)%8==0) {dlc=lta+0;}
else if((lta+2)%8==0) {dlc=lta+2;}
else if((lta+4)%8==0) {dlc=lta+4;}
else if((lta+6)%8==0) {dlc=lta+6;}
tolines(dlc,cpr); 
this.cc=cpr;

this.lc=(int)((tlc/dlc)+0.4);
System.out.println("tlc: "+tlc+" tcc: "+tcc+" cpr: "+cpr+" lta: "+lta+" dlc: "+dlc+" lc: "+lc+" cc: "+cc);}
...
try {
BufferedReader br = new BufferedReader(new FileReader(bsf));
BufferedWriter bw = new BufferedWriter(new FileWriter(com,true)); 
for(int y=0;y<lc;y++) {
line = br.readLine();
if(line!=null) {
for(int x=0;x<line.length()/4;x++) {
str=line.substring(start,end);start=end;end=end+4; 
enc=encodeString(str);bw.write(enc);}
start=0;end=4;line="";str="";bw.newLine();}}br.close();bw.close();
}catch (IOException e) {e.printStackTrace();}} 

public void decode(File input, File output)throws IOException{
int start=0;int end=8;
String line;String str;String dec;
try {
BufferedReader br = new BufferedReader(new FileReader(this.com));
BufferedWriter bw = new BufferedWriter(new FileWriter(this.dec,true)); 
for(int y=0;y<lc;y++) {
line = br.readLine();
if(line!=null) {
for(int x=0;x<line.length()/8;x++) {
str=line.substring(start,end);start=end;end=end+8; 
dec=decodeString(str);bw.write(dec);}
start=0;end=8;line="";str="";bw.newLine();}}br.close();bw.close();
}catch (IOException e) {e.printStackTrace();}} 

public void DECtoOUT(File input, File output) throws IOException {
